...
const mockReviewsPath = path.join(__dirname, '../src/data/mockReviews.ts');
let content = fs.readFileSync(mockReviewsPath, 'utf8');

// Update interfaces
if (!content.includes('content_ja?:')) {
  content = content.replace(
    /export interface Comment \{([\s\S]*?)\}/,
    (match, p1) => {
      return `export interface Comment {${p1}
  content_ja?: string;
  ...
  content = content.replace(
    /export interface Review \{([\s\S]*?)\}/,
    (match, p1) => {
      return `export interface Review {${p1}
  location_ja?: string;
...
// Inject translations for each review
const translations = {
  1: {
...
for (const [id, tr] of Object.entries(translations)) {
  const locRegex = new RegExp("(id: " + id + ",[\\s\\S]*?location_zh_TW:.*?,)", "g");
  content = content.replace(locRegex, "$1\n    location_ja: " + JSON.stringify(tr.location_ja) + ",\n    location_zh_CN: " + JSON.stringify(tr.location_zh_CN) + ",");

  const contentRegex = new RegExp("(id: " + id + ",[\\s\\S]*?content_zh_TW:\\s*\"[^\"]*\",)", "g");
  content = content.replace(contentRegex, "$1\n    content_ja: " + JSON.stringify(tr.content_ja) + ",\n    content_zh_CN: " + JSON.stringify(tr.content_zh_CN) + ",");
}

if (!content.includes("export function getLocalizedField")) {
...
console.log('Successfully injected Japanese & Simplified Chinese into mockReviews.ts');
