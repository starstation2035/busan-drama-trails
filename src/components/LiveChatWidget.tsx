...
export function LiveChatWidget({ isOpen, onClose }: LiveChatWidgetProps) {
  ...
  return (
    <div
      className={`fixed bottom-0 right-0 z-[70] flex flex-col overflow-hidden bg-[#b2c7d9] shadow-2xl transition-all duration-300 ease-in-out sm:right-6 sm:rounded-2xl border border-white/20 animate-fade-up ${isMinimized ? "h-[50px] w-[240px] sm:bottom-6" : "h-[80vh] w-full sm:bottom-6 sm:h-[420px] sm:w-[360px]"}`}
    >
      {/* Header */}
      <div
          ...
          {!userProfile ? (
            ...
            <div className="flex flex-1 flex-col overflow-y-auto custom-scrollbar p-4 text-[#3b1e1e] animate-fade-in">
              <div className="flex flex-col items-center mb-3">
                ...
                <h2 className="mb-0 text-base font-black uppercase tracking-tight">
                  {t("chat.setup.title") || "Traveler Profile"}
                </h2>
              </div>

              <form onSubmit={(e) => handleJoin(e)} className="w-full space-y-3">
                <div className="space-y-1">
                  <label className="ml-1 text-[9px] font-bold uppercase opacity-60 tracking-wider">
                    Nickname
                  </label>
                  <input
                    type="text"
            ...
            <div className="flex flex-1 flex-col p-4 animate-fade-in">
              <div className="flex items-center justify-between mb-4 px-2">
                <h3 className="text-sm font-bold text-[#3b1e1e] uppercase tracking-wider">
                  Online Travelers
                </h3>
                <button
                  onClick={() => setShowUserList(false)}
                  className="text-xs font-bold text-[#3b1e1e]/60 hover:text-[#3b1e1e]"
                >
                  Close
                </button>
              </div>
              <div className="flex-1 space-y-2 overflow-y-auto pr-1 [scrollbar-width:none]">
                ...
                <div className="flex items-center gap-3 rounded-2xl bg-white/40 p-3 border border-white/20">
                  ...
                  <div className="flex flex-1 flex-col">
                    ...
                    <span className="text-[10px] opacity-60 font-medium">
                      {COUNTRIES.find((c) => c.code === userProfile.country)?.flag}{" "}
                ...
                {onlineUsers.map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center gap-3 rounded-2xl bg-white/20 p-3 border border-white/10"
                  >
            ...
            <>
              <div
                className="flex-1 overflow-y-auto p-4 space-y-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                ref={scrollRef}
              >
                ...
                {messages.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`flex w-full ${isMe ? "justify-end" : "justify-start"} animate-slide-in`}
                    >
                      {!isMe && (
                        <img
                          src={msg.avatar}
                          alt="avatar"
                          className="h-9 w-9 rounded-[14px] shadow-sm border border-black/5 mr-2 mt-1"
                        />
                      )}
                      <div
                      ...
                      >
                        {!isMe && (
                          <div className="mb-1 ml-1 flex items-center gap-1">
                            <span className="text-[11px] font-semibold text-[#3b1e1e]/70">
                              {msg.senderName}
                            </span>
                        ...
                        <div
                          className={`relative px-3 py-2 text-[13px] leading-relaxed shadow-sm ${isMe ? "rounded-l-[18px] rounded-br-[4px] rounded-tr-[18px] bg-[#FAE100] text-[#3b1e1e]" : "rounded-r-[18px] rounded-bl-[4px] rounded-tl-[18px] bg-white text-[#3b1e1e]"}`}
                        >
                          {msg.translatedContent ? (
                            <div className="flex flex-col gap-1.5">
                              <span className="font-medium text-[#3b1e1e] border-b border-[#3b1e1e]/10 pb-1.5 mb-0.5">
                                {msg.translatedContent}
                              </span>
                              <span className="text-[11px] opacity-60">
                                {t("chat.original")}: {msg.content}
                              </span>
                            </div>
                          ) : (
                        ...
                        <div
                          className={`mt-1 flex items-center gap-2 px-1 text-[10px] text-[#3b1e1e]/50 ${isMe ? "flex-row-reverse" : ""}`}
                        >
                          <span>
                          ...
                          {!isMe && (
                            <button
                              onClick={() =>
                                toggleTranslate(msg.id, i18n.language.split("-")[0] || "ko")
                              }
