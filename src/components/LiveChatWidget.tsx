    ...
    <>
      ...
      {!isOpen && (
        <button 
          ...
          className="fixed bottom-24 right-6 md:bottom-10 z-[60] flex h-14 w-14 animate-bounce items-center justify-center rounded-full bg-[#FAE100] text-[#3B1E1E] shadow-2xl shadow-[#FAE100]/40 transition-transform active:scale-95"
        >
      ...
      {isOpen && (
        <div className="fixed bottom-0 right-0 z-[70] flex h-[85vh] w-full flex-col overflow-hidden bg-[#b2c7d9] shadow-2xl transition-all sm:bottom-24 sm:right-6 sm:h-[600px] sm:w-[380px] sm:rounded-3xl border border-white/20 animate-fade-up">
          ...
          <div className="flex flex-shrink-0 items-center justify-between bg-black/10 px-4 py-3 backdrop-blur-md">
            ...
            <button onClick={() => setIsOpen(false)} className="rounded-full bg-white/20 p-2 text-[#3b1e1e] hover:bg-white/40 transition-colors">
              <X className="h-4 w-4" />
            </button>
          </div>
