      ...
      <DrawerContent className="max-h-[92vh] rounded-t-[32px] border-none bg-background shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto pb-10 scrollbar-hide">
          ...
          <div className="relative w-full h-[280px] sm:h-[340px]">
            ...
            >
              <Heart
                className={cn("h-7 w-7 transition-all duration-300", isFavorite ? "fill-red-500 text-red-500 scale-110" : "text-gray-400")}
              />
            </button>
            ...
            <div className="absolute bottom-6 left-6 z-10">
              <div className="flex items-center space-x-2 text-white/90 text-sm font-medium mb-1 backdrop-blur-sm bg-black/20 px-2.5 py-1 rounded-full w-fit">
                {isRestaurant ? <Utensils className="h-3.5 w-3.5" /> : <Coffee className="h-3.5 w-3.5" />}
                <span>{typeText}</span>
              </div>
          ...
          <div className="px-6 py-6 space-y-8">
            ...
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-2xl border border-border/50">
              ...
              <div className="flex flex-col items-center flex-1">
                ...
                <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">Price Range</span>
              </div>
            </div>
            ...
            {place.images && place.images.length > 0 && (
              <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-muted-foreground font-medium">{place.images.length} photos</span>
                </div>
                <div className="overflow-hidden -mx-6 px-6" ref={emblaRef}>
                  <div className="flex gap-3">
                    {place.images.map((img, i) => (
                      <div key={i} className="flex-[0_0_200px] sm:flex-[0_0_240px] aspect-[4/3] rounded-2xl overflow-hidden shadow-md ring-1 ring-border/50">
                        <img src={img} alt={`Food ${i+1}`} className="w-full h-full object-cover transition-transform hover:scale-110 duration-500" loading="lazy" />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {signatureText && (
              <section className="bg-gradient-to-br from-primary/10 to-primary/5 p-5 rounded-2xl border border-primary/20">
                <div className="flex items-center space-x-2 text-primary mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider">Expert's Choice</span>
                </div>
                  ...
                  {place.menu.map((m, i) => (
                    <div key={i} className={cn(
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-base text-foreground">{m.item[lang] || m.item["ko"]}</span>
                        <span className="text-xs text-muted-foreground uppercase font-medium">{m.item["en"]}</span>
                      </div>
