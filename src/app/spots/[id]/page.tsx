...
export default function SpotDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isFav = spot ? favorites.includes(spot.id) : false;
  ...
  if (!spot) {
    return (
      <div className="py-20 text-center">
        ...
        <Link href="/spots" className="mt-4 inline-block text-primary underline">
          Back to all spots
        </Link>
      </div>
    );
  ...
  const restaurants = (spot.nearby_restaurants ?? [])
    .map((rid) => RESTAURANTS.find((r) => r.id === rid))
    .filter((r): r is Restaurant => Boolean(r));
    
  const cafes = (spot.nearby_cafes ?? [])
    .map((cid) => CAFES.find((c) => c.id === cid))
    .filter((c): c is Cafe => Boolean(c));

  const goBack = () => {
    if (window.history.length > 1) router.back();
    else router.push("/spots");
  };
      ...
      >
        <div className="mx-auto max-w-screen-md">
          {isFav ? (
            ...
            >
              <Link href="/my-course">{t("detail.cta.added")}</Link>
            </Button>
          ) : (
            <Button
              size="lg"
